...
impl Iterator for FoldIter {
    ...
    fn next(&mut self) -> Option<char> {
        match self {
            FoldIter::Char(c) => c.take(),
            FoldIter::Chars(it) => it.next(),
        }
    }
}

pub fn fold_char_to_ascii(chr: char) -> impl Iterator<Item = char> {
    if chr.is_ascii() {
        return FoldIter::Char(Some(chr));
    }

    let folded: &'static str = match chr {
        ...
        => "~",
        _ => return FoldIter::Char(Some(chr))
    };

    FoldIter::Chars(folded.chars())
}

pub fn fold_to_ascii(input: &str) -> String {
    let mut output = String::with_capacity(input.len());
    fold_to_ascii_buffer(input, &mut output);
    output
}

pub fn fold_to_ascii_buffer(input: &str, output: &mut String) {
...
#[cfg(test)]
mod tests {
    use super::{fold_char_to_ascii, fold_to_ascii};

    fn fold(input: &str) -> String {
        use std::iter::FromIterator;

        let direct = fold_to_ascii(input);
        let iter = String::from_iter(input.chars().flat_map(fold_char_to_ascii));
        assert_eq!(direct, iter);
        direct
    }
    ...
    fn test_latin1_accents() {
        ...
        for (input, expected) in foldings {
    ...
    fn test_all_foldings() {
        let foldings = vec![
            (
                r#"ÀÁÂÃÄÅĀĂĄƏǍǞǠǺȀȂȦȺᴀḀẠẢẤẦẨẪẬẮẰẲẴẶⒶＡ"#,
                r#"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"#,
            ),
            (
                r#"àáâãäåāăąǎǟǡǻȁȃȧɐəɚᶏḁᶕẚạảấầẩẫậắằẳẵặₐₔⓐⱥⱯａ"#,
                r#"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"#,
            ),
            (r#"Ꜳ"#, r#"AA"#),
            (r#"ÆǢǼᴁ"#, r#"AEAEAEAE"#),
            (r#"Ꜵ"#, r#"AO"#),
            ...
            (r#"ꜹꜻ"#, r#"avav"#),
            (r#"ꜽ"#, r#"ay"#),
            (r#"ƁƂɃʙᴃḂḄḆⒷＢ"#, r#"BBBBBBBBBB"#),
            (r#"ƀƃɓᵬᶀḃḅḇⓑｂ"#, r#"bbbbbbbbbb"#),
            (r#"⒝"#, r#"(b)"#),
            (r#"ÇĆĈĊČƇȻʗᴄḈⒸＣ"#, r#"CCCCCCCCCCCC"#),
            (r#"çćĉċčƈȼɕḉↄⓒꜾꜿｃ"#, r#"cccccccccccccc"#),
            (r#"⒞"#, r#"(c)"#),
            (
                r#"ÐĎĐƉƊƋᴅᴆḊḌḎḐḒⒹꝹＤ"#,
                r#"DDDDDDDDDDDDDDDD"#,
            ...
            (r#"ǆǳʣʥ"#, r#"dzdzdzdz"#),
            (
                r#"ÈÉÊËĒĔĖĘĚƎƐȄȆȨɆᴇḔḖḘḚḜẸẺẼẾỀỂỄỆⒺⱻＥ"#,
                r#"EEEEEEEEEEEEEEEEEEEEEEEEEEEEEEEE"#,
            ),
            (
                r#"èéêëēĕėęěǝȅȇȩɇɘɛɜɝɞʚᴈᶒᶓᶔḕḗḙḛḝẹẻẽếềểễệₑⓔⱸｅ"#,
                r#"eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"#,
            ),
            (r#"⒠"#, r#"(e)"#),
            (r#"ƑḞⒻꜰꝻꟻＦ"#, r#"FFFFFFF"#),
            (r#"ƒᵮᶂḟẛⓕꝼｆ"#, r#"ffffffff"#),
            (r#"⒡"#, r#"(f)"#),
            (r#"ﬀ"#, r#"ff"#),
            (r#"ﬃ"#, r#"ffi"#),
            (r#"ﬄ"#, r#"ffl"#),
            ...
            (r#"⒢"#, r#"(g)"#),
            (r#"ĤĦȞʜḢḤḦḨḪⒽⱧⱵＨ"#, r#"HHHHHHHHHHHHH"#),
            (
                r#"ĥħȟɥɦʮʯḣḥḧḩḫẖⓗⱨⱶｈ"#,
                r#"hhhhhhhhhhhhhhhhh"#,
            ),
            ...
            (r#"ƕ"#, r#"hv"#),
            (
                r#"ÌÍÎÏĨĪĬĮİƖƗǏȈȊɪᵻḬḮỈỊⒾꟾＩ"#,
                r#"IIIIIIIIIIIIIIIIIIIIIII"#,
            ),
            (
                r#"ìíîïĩīĭįıǐȉȋɨᴉᵢᵼᶖḭḯỉịⁱⓘｉ"#,
                r#"iiiiiiiiiiiiiiiiiiiiiiii"#,
            ),
            (r#"Ĳ"#, r#"IJ"#),
            (r#"⒤"#, r#"(i)"#),
            (r#"ĳ"#, r#"ij"#),
            (r#"ĴɈᴊⒿＪ"#, r#"JJJJJ"#),
            (r#"ĵǰȷɉɟʄʝⓙⱼｊ"#, r#"jjjjjjjjjj"#),
            (r#"⒥"#, r#"(j)"#),
            (
                r#"ĶƘǨᴋḰḲḴⓀⱩꝀꝂꝄＫ"#,
                r#"KKKKKKKKKKKKK"#,
            ...
            (r#"⒦"#, r#"(k)"#),
            (
                r#"ĹĻĽĿŁȽʟᴌḶḸḺḼⓁⱠⱢꝆꝈꞀＬ"#,
                r#"LLLLLLLLLLLLLLLLLLL"#,
            ),
            (
                r#"ĺļľŀłƚȴɫɬɭᶅḷḹḻḽⓛⱡꝇꝉꞁｌ"#,
            ...
            (r#"ʪ"#, r#"ls"#),
            (r#"ʫ"#, r#"lz"#),
            (r#"ƜᴍḾṀṂⓂⱮꟽꟿＭ"#, r#"MMMMMMMMMM"#),
            (r#"ɯɰɱᵯᶆḿṁṃⓜｍ"#, r#"mmmmmmmmmm"#),
            (r#"⒨"#, r#"(m)"#),
            (
                r#"ÑŃŅŇŊƝǸȠɴᴎṄṆṈṊⓃＮ"#,
                r#"NNNNNNNNNNNNNNNN"#,
            ...
            (
                r#"ñńņňŉŋƞǹȵɲɳᵰᶇṅṇṉṋⁿⓝｎ"#,
                r#"nnnnnnnnnnnnnnnnnnnn"#,
            ),
            (r#"Ǌ"#, r#"NJ"#),
            (r#"ǋ"#, r#"Nj"#),
            (r#"⒩"#, r#"(n)"#),
            (r#"ǌ"#, r#"nj"#),
            (
                r#"ÒÓÔÕÖØŌŎŐƆƟƠǑǪǬǾȌȎȪȬȮȰᴏᴐṌṎṐṒỌỎỐỒỔỖỘỚỜỞỠỢⓄꝊꝌＯ"#,
                r#"OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO"#,
            ),
            (
                r#"òóôõöøōŏőơǒǫǭǿȍȏȫȭȯȱɔɵᴖᴗᶗṍṏṑṓọỏốồổỗộớờởỡợₒⓞⱺꝋꝍｏ"#,
                r#"ooooooooooooooooooooooooooooooooooooooooooooooo"#,
            ),
            (r#"Œɶ"#, r#"OEOE"#),
            (r#"Ꝏ"#, r#"OO"#),
            (r#"Ȣᴕ"#, r#"OUOU"#),
            ...
            (r#"ꝏ"#, r#"oo"#),
            (r#"ȣ"#, r#"ou"#),
            (r#"ƤᴘṔṖⓅⱣꝐꝒꝔＰ"#, r#"PPPPPPPPPP"#),
            (r#"ƥᵱᵽᶈṕṗⓟꝑꝓꝕꟼｐ"#, r#"pppppppppppp"#),
            (r#"⒫"#, r#"(p)"#),
            (r#"ɊⓆꝖꝘＱ"#, r#"QQQQQ"#),
            (r#"ĸɋʠⓠꝗꝙｑ"#, r#"qqqqqqq"#),
            (r#"⒬"#, r#"(q)"#),
            (r#"ȹ"#, r#"qp"#),
            (
                r#"ŔŖŘȐȒɌʀʁᴙᴚṘṚṜṞⓇⱤꝚꞂＲ"#,
                r#"RRRRRRRRRRRRRRRRRRR"#,
            ),
            (
                r#"ŕŗřȑȓɍɼɽɾɿᵣᵲᵳᶉṙṛṝṟⓡꝛꞃｒ"#,
            ...
            (
                r#"śŝşšſșȿʂᵴᶊṡṣṥṧṩẜẝⓢꞄｓ"#,
                r#"ssssssssssssssssssss"#,
            ),
            (r#"ẞ"#, r#"SS"#),
            (r#"⒮"#, r#"(s)"#),
            (r#"ß"#, r#"ss"#),
            ...
            (
                r#"ţťŧƫƭțȶʇʈᵵṫṭṯṱẗⓣⱦｔ"#,
                r#"tttttttttttttttttt"#,
            ),
            (r#"ÞꝦ"#, r#"THTH"#),
            (r#"Ꜩ"#, r#"TZ"#),
            (r#"⒯"#, r#"(t)"#),
            ...
            (r#"ꜩ"#, r#"tz"#),
            (
                r#"ÙÚÛÜŨŪŬŮŰŲƯǓǕǗǙǛȔȖɄᴜᵾṲṴṶṸṺỤỦỨỪỬỮỰⓊＵ"#,
                r#"UUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUUU"#,
            ),
            (
                r#"ùúûüũūŭůűųưǔǖǘǚǜȕȗʉᵤᶙṳṵṷṹṻụủứừửữựⓤｕ"#,
                r#"uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu"#,
            ),
            (r#"⒰"#, r#"(u)"#),
            (r#"ᵫ"#, r#"ue"#),
            (r#"ƲɅᴠṼṾỼⓋꝞꝨＶ"#, r#"VVVVVVVVVV"#),
            (r#"ʋʌᵥᶌṽṿⓥⱱⱴꝟｖ"#, r#"vvvvvvvvvvv"#),
            (r#"Ꝡ"#, r#"VY"#),
            (r#"⒱"#, r#"(v)"#),
            (r#"ꝡ"#, r#"vy"#),
            (r#"ŴǷᴡẀẂẄẆẈⓌⱲＷ"#, r#"WWWWWWWWWWW"#),
            (r#"ŵƿʍẁẃẅẇẉẘⓦⱳｗ"#, r#"wwwwwwwwwwww"#),
            (r#"⒲"#, r#"(w)"#),
            (r#"ẊẌⓍＸ"#, r#"XXXX"#),
            (r#"ᶍẋẍₓⓧｘ"#, r#"xxxxxx"#),
            (r#"⒳"#, r#"(x)"#),
